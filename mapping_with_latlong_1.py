...
with open('mapped_zill_abnb_ScottsDale_data.csv', mode='w', newline='') as file:
        ...
        for zill_index, zill_row in zill_df.iterrows():
            ...
            for abnb_image in image_links_abnb:
                for zil_image in image_links_zill:
                    ...
                    if generateScore(abnb_image, zil_image) >= 80: # similarity score
                        count += 1
                        print("----image mapped----")
                        if count >= 2:
                            data = [zill_row.get('Listing_link'), abnb_row.get('listing_link'), 
                                    zill_row.get('Property_address'),zill_row.get('Rent_price'),
                                    zill_row.get('#bedrooms'), zill_row.get('#bathrooms'),
                                    zill_row.get('Square_footage')]
...
def map():
    ...
    with open('similar_properties_under_20_meter.csv', mode='w', newline='') as file:
        ...
        for zill_index, zill_row in zill_df.iterrows():
            ...
            if distance < 50: # if distance between airbnb and zillow property is less than 100 we will map their images for similarity
                ...
                if distance <= 20:
                    ...
                    with open('similar_properties_under_20_meter.csv', 'a', newline='') as file:
                        writer = csv.writer(file)
                        writer.writerow(zill_data)

        if num >= 1: # call map function only if there is atleast one property under 50 meter dustance
            mapping_func()
